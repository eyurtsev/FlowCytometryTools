...
doc_replacer.update(
    _gate_pars_2_channels="""channels : ['channel 1 name', 'channel 2 name']
                        Defines the names of the channels"""
)

doc_replacer.update(
    _gate_plot_doc="""Plots the gate.
...
class Gate(_ComposableMixin):
    ...
    def __repr__(self):
        return """Gate Type: {0}
               ...
               \tName: {3}
                """.format(
            self.__class__, self.vert, self.channels, self.name
        )
            ...
            try:
                ...
                raise Exception(
                    """Trying to plot gate that is defined on channel {0},
                                but figure axis correspond to channels {1}""".format(
                        c, ax_channels
                    )
                )
...
class ThresholdGate(Gate):
    ...
    def _identify(self, dataframe):
        ...
        idx = (
            dataframe[self.channels[0]] >= self.vert
        )  # Get indexes that are above threshold
...
class IntervalGate(Gate):
    ...
    def validate_input(self):
        ...
        if self.vert[1] <= self.vert[0]:
            raise ValueError(
                "{} must be larger than {}".format(self.vert[1], self.vert[0])
            )
...
class CompositeGate(_ComposableMixin):
    ...
    def name(self):
        if len(self.gates) == 1:
            ...
            return "{0} {1} {2}".format(
                self.gates[0].name, self.how, self.gates[1].name
            )
    ...
    def _identify(self, dataframe):
        ...
        if self.how == "and":
            ...
            raise ValueError(
                "Unsupported value for how. how must be in ({0})".format(
                    supported_values
                )
            )
